...
class ShannonDecomposition(KCRule):
    ...
    def is_applicable(cls, cnf: 'CNF') -> Tuple[bool, Optional['ConstrainedAtom']]:
        ...
        for clause in sorted(cnf.clauses):
            for c_atom in sorted(clause.get_constrained_atoms()):
                overlapping_terms = c_atom.bound_vars.intersection(set(c_atom.atom.terms))
    ...
    def apply(cls, cnf: 'CNF', unbound_atom: 'ConstrainedAtom', compiler: 'Compiler') -> 'NNFNode':
        ...
        # NOTE: these 'literals' are technically a single-item set of literals, i.e. literals = (literal)
        ...
        # (e.g. ¬[10 < age(X) < 20] goes to -inf < age(X) < 10 v 20 < age(X) < inf)
            ...
            new_low_predicate = SMTPredicate(old_predicate.name,
                                             old_predicate.arity,
                                             float('-inf'),
                                             old_predicate.lower_bound)
            new_high_predicate = SMTPredicate(old_predicate.name,
                                              old_predicate.arity,
                                              old_predicate.upper_bound,
                                              float('inf'))
