...
atom2 = Atom(pred, [Y, Z, a])

XeqY = EqualityConstraint(X, Y)
Yneqa = InequalityConstraint(X, a)
XinD = InclusionConstraint(X, D)
YinD = InclusionConstraint(Y, D)
ZinD = InclusionConstraint(Z, D)
cs = ConstraintSet([XeqY, Yneqa, XinD, YinD, ZinD])
c_atom1 = ConstrainedAtom(UnconstrainedClause([Literal(atom1, True)]), [X], cs)
c_atom2 = ConstrainedAtom(UnconstrainedClause([Literal(atom2, True)]), [Y, Z], cs)
