...
def condition(gamma: 'ConstrainedClause', literal: 'UnitClause') -> Optional['ConstrainedClause']:
    ...
    if constrained_clauses_subsumed(literal, gamma): # gamma is redundant when we have 'literal'
        return None
    else:
        necessary_literals = discard_unsatisfied_literals(gamma, literal)
        Lambda = ConstrainedClause(UnconstrainedClause(necessary_literals), gamma.bound_vars, gamma.cs)
        return Lambda


def discard_unsatisfied_literals(gamma: 'ConstrainedClause', literal: 'UnitClause') -> List['Literal']:
