...
def test_atom():
    returned_atom = atom.apply_substitution(subXa)
    target_atom = Atom(likes, [a, a])
    assert(returned_atom == target_atom)

def test_literal():
    returned_literal = notlikesXa.apply_substitution(subXa)
    target_literal = Literal(Atom(likes, [a, a]), False)
    assert(returned_literal == target_literal)

def test_uclause():
    returned_uclause = uclause.apply_substitution(subYX)
...
def test_cs():
    returned_cs = cs.apply_substitution(subYX)
...
def test_cclause():
    returned_cclause = cclause.apply_substitution(subYX)
    friendsXX = Literal(Atom(friends, [X, X]), True)
    target_cclause = ConstrainedClause([notlikesXa, friendsXX], [X], cs.apply_substitution(subYX))
    assert(returned_cclause == target_cclause)

def test_cnf():
    returned_cnf = cnf.apply_substitution(subYX)
    target_cnf = CNF([uclause.apply_substitution(subYX), cclause.apply_substitution(subYX)])
